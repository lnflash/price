...
test("an advisory with no GHSA slug is keyed by yarn's numeric advisory id", () => {
  ...
  // Composite (id, module) key: the id half falls back to yarn's numeric id.
  assert.deepStrictEqual([...advisories.keys()], ["1234\u0000some-pkg"])
})
...
test("a waiver whose module does not match the advisory fails", () => {
  ...
  assert.strictEqual(waived.length, 0)
  // Two failures, and both are wanted: the waiver names the wrong package, AND
  // the advisory it failed to cover is now unwaived. Reporting only the first
  // would leave a blocking advisory unmentioned in the gate's own output.
  assert.strictEqual(failures.length, 2)
  assert.match(
    failures[0],
    /Waiver for GHSA-waived-0001 claims module some-pkg, but that advisory is against other-pkg/,
  )
  assert.match(failures[1], /HIGH other-pkg \(GHSA-waived-0001\)/)
})

test("a waiver with the wrong module is told so, not told its date expired", () => {
  // Both wrong at once. The module is the prerequisite: until the waiver names
  // the package it actually covers, "extend the review date" is advice about a
  // waiver that is not covering anything.
  const { failures, waived } = evaluate({
    advisories: advisory({ id: "GHSA-waived-0001", module: "other-pkg" }),
    summary,
  ...
  assert.strictEqual(waived.length, 0)
  assert.ok(failures.some((f) => /claims module some-pkg, but that advisory is against other-pkg/.test(f)))
  assert.ok(
    !failures.some((f) => /expired on/.test(f)),
    "the module mismatch is the actionable failure; the expiry note would be noise",
  )
})

test("one advisory id published against two packages is kept apart", () => {
  // Verified against this repo's real audit: GHSA-968p-4wvh-cqc8 arrives for
  // both @babel/helpers and @babel/runtime. Keyed on the id alone the two
  // collapse into one entry, and a waiver for either silently accepts both.
  const advisories = collectAdvisory(
    advisory({ id: "GHSA-shared-0001", module: "pkg-a" }),
    advisoryData({ id: "GHSA-shared-0001", module: "pkg-b" }),
  )
  assert.strictEqual(advisories.size, 2)

  const { failures, waived } = evaluate({
    advisories,
    summary: { critical: 0, high: 2, moderate: 0, low: 0 },
    waivers: [waiver({ id: "GHSA-shared-0001", module: "pkg-a" })],
    today: TODAY,
  })
  // pkg-a is waived; pkg-b is a different package and must still block.
  assert.strictEqual(waived.length, 1)
  assert.strictEqual(waived[0].module, "pkg-a")
  assert.strictEqual(failures.length, 1)
  assert.match(failures[0], /HIGH pkg-b \(GHSA-shared-0001\)/)
})
